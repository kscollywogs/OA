...
public class ManageUserServlet extends BaseServlet {
    ...
    public void findAllUser(HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("/dist/root/modify.jsp").forward(request,response);
    }
    ...
    public void addUser(HttpServletRequest request, HttpServletResponse response)
        ...
        if(b){
            ...
            request.getRequestDispatcher("/dist/root/register.jsp").forward(request, response);
            return;
        }
        ...
        if(i>0){
            ...
            out.print("<script>alert('添加成功！');window.location='"+request.getContextPath()+"/dist/root/register.jsp';</script>");
        }else{
            PrintWriter out = response.getWriter();
            out.print("<script>alert('添加失败！');window.location='"+request.getContextPath()+"/dist/root/register.jsp';</script>");
        }
    }
