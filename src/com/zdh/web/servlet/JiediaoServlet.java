...
public class JiediaoServlet extends BaseServlet {
    ...
    public void addJiediao(HttpServletRequest request, HttpServletResponse response)
        ...
        if(i>0 && j>0){
            out.print("<script>alert('修改借调状态成功！');window.location='"+request.getContextPath()+"/dist/forms/transfer.jsp';</script>");
        }else{
            out.print("<script>alert('修改借调状态失败！');window.location='"+request.getContextPath()+"/dist/forms/transfer.jsp';</script>");
        }
